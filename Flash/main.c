...
byte read_eep(byte address_byte) 
{
    ...
    byte data;	                // Определяем переменную data, в которой будет
                                // храниться считанное значение 
    i2c_start();	            // Генерируем стартовое условие
    i2c_write(addres_eep);      // Передаём в шину I2C адрес EEPROM с младшим
                                // байтом = 0, т. к. следующая операция - запись                         
    i2c_write(0x00);            // Передаём в шину I2C адрес байта,               
    i2c_write(address_byte);    // который требуется прочитать                                 
    i2c_start();	            // Генерируем стартовое условие
    i2c_write(addres_eep | 1);   // Изменяем самый младший байт адреса
                                 // EEPROM на 1, т. к. будем производить 
                                 // операцию чтени
    data=i2c_read(0);           // Читаем байт из шины I2C в переменную dat           	        
    i2c_stop();	                // Генерируем стоповое условие
    return data;	            // Возвращаем значение переменной data
...
void write_eep(byte address_byte, byte data) 
{
    ...
    i2c_start();	            //	Генерируем стартовое условие
    i2c_write(addres_eep);      //	Передаём в шину I2C адрес EEPROM            
    i2c_write(0x00);            //	Передаём в шину I2C адрес ячейки,
    i2c_write(address_byte);    //	куда требуется записать байт                            
    i2c_write(data);           //	Записываем байт данных data
    i2c_stop();	                //	Генерируем стоповое условие
    delay_ms(10);	            //	завершаем операцию записи задержкой 10 мс
