...
_0x0:
	.DB  0xA8,0xEB,0xE5,0xEA,0xF2,0xF0,0xEE,0xED
	.DB  0xE8,0xEA,0xE0,0x20,0xE2,0x20,0xEE,0xE1
	.DB  0xFA,0xE5,0xEA,0xF2,0xE8,0xE2,0xE5,0x0
_0x2040060:
	.DB  0x1
_0x2040000:
...
; 0000 002C // Function: Bit7=In Bit6=In Bit5=Out Bit4=In Bit3=Out Bit2=Out Bit1=In Bit0=In
; 0000 002D DDRB=(0<<DDB7) | (0<<DDB6) | (1<<DDB5) | (0<<DDB4) | (1<<DDB3) | (1<<DDB2) | (0<<DDB1) | (0<<DDB0);
	LDI  R30,LOW(44)
	OUT  0x17,R30
; 0000 002E // State: Bit7=T Bit6=T Bit5=0 Bit4=T Bit3=0 Bit2=0 Bit1=T Bit0=T
; 0000 002F PORTB=(0<<PORTB7) | (0<<PORTB6) | (0<<PORTB5) | (0<<PORTB4) | (0<<PORTB3) | (0<<PORTB2) | (0<<PORTB1) | (0<<PORTB0);
	LDI  R30,LOW(0)
	OUT  0x18,R30
; 0000 0030 
; 0000 0031 // Port C initialization
; 0000 0032 // Function: Bit6=In Bit5=In Bit4=In Bit3=In Bit2=In Bit1=In Bit0=In
; 0000 0033 DDRC=(0<<DDC6) | (0<<DDC5) | (0<<DDC4) | (0<<DDC3) | (0<<DDC2) | (0<<DDC1) | (0<<DDC0);
	OUT  0x14,R30
; 0000 0034 // State: Bit6=T Bit5=T Bit4=T Bit3=T Bit2=T Bit1=T Bit0=T
...
; 0000 0040 TCCR0=(0<<CS02) | (0<<CS01) | (0<<CS00);
	OUT  0x33,R30
...
; 0000 007B ADCSRA=(0<<ADEN) | (0<<ADSC) | (0<<ADFR) | (0<<ADIF) | (0<<ADIE) | (0<<ADPS2) | (0<<ADPS1) | (0<<ADPS0);
	OUT  0x6,R30
; 0000 007C 
; 0000 007D // SPI initialization
; 0000 007E // SPI Type: Master
; 0000 007F // SPI Clock Rate: 500,000 kHz
; 0000 0080 // SPI Clock Phase: Cycle Start
; 0000 0081 // SPI Clock Polarity: Low
; 0000 0082 // SPI Data Order: MSB First
; 0000 0083 SPCR=(0<<SPIE) | (1<<SPE) | (0<<DORD) | (1<<MSTR) | (0<<CPOL) | (0<<CPHA) | (0<<SPR1) | (1<<SPR0);
	LDI  R30,LOW(81)
	OUT  0xD,R30
...
; 0000 0089 
; 0000 008A 
; 0000 008B  // »нициализируем светодиодную матрицу
; 0000 008C   ledmatrix_init();
	RCALL _ledmatrix_init
; 0000 008D 
; 0000 008E   // ћигнЄм всеми матрицами (типа тест индикаторов)
; 0000 008F   ledmatrix_testmatrix(1);
	LDI  R26,LOW(1)
...
; 0000 0090   delay_ms(500);
	...
	LDI  R27,HIGH(500)
	RCALL _delay_ms
; 0000 0091   ledmatrix_testmatrix(0);
	LDI  R26,LOW(0)
	RCALL _ledmatrix_testmatrix
; 0000 0092 
; 0000 0093 while (1)
...
; 0000 0096       // ‘ормируем буфер дл¤ скролла на матрице с желаемым текстом
; 0000 0097         // ?л¤ форматированного вывода можно также использовать функцию font_printf
; 0000 0098         uint16_t ScrollLines = font_DrawString("Ёлектроника в объективе", ScrollBuff, 140);
; 0000 0099         // —кроллим текст по матрице справа налево
; 0000 009A         demo_ScrollBuff(ScrollBuff, ScrollLines, 1);
	SBIW R28,2
;	ScrollLines -> Y+0
	__POINTW1MN _0x6,0
	RCALL SUBOPT_0x0
...
; 0000 009B         // —кроллим текст по матрице слева направо
; 0000 009C         demo_ScrollBuff(ScrollBuff, ScrollLines, 0);
	RCALL SUBOPT_0x1
	LDD  R30,Y+2
	LDD  R31,Y+2+1
	RCALL SUBOPT_0x0
...
; 0003 0016    MAX7219_CS_DDR |= (1 << MAX7219_CS_PIN);
	SBI  0x17,0
; 0003 0017 
; 0003 0018    MAX7219_CS_PORT |= (1 << MAX7219_CS_PIN);
	SBI  0x18,0
; 0003 0019    MAX7219_CS_HIGHT();
	SBI  0x18,0
; 0003 001A }
	RET
; .FEND
...
; 0003 0023   uint16_t max7219_SpiBuff[MAX7219_NUM];
; 0003 0024   uint16_t Word = Data | ((uint16_t) Cmd << 8);
; 0003 0025   uint8_t i;
; 0003 0026 
; 0003 0027   for (i = 0; i < MAX7219_NUM; i++)
	ST   -Y,R26
	SBIW R28,8
	RCALL __SAVELOCR4
;	MAX_Idx -> Y+14
...
_0x60004:
	...
	BRSH _0x60005
; 0003 0028   {
; 0003 0029     if (MAX_Idx == 0xFF)  // Нужно записать во все max7219 в цепочке
	LDD  R26,Y+14
	CPI  R26,LOW(0xFF)
	BRNE _0x60006
; 0003 002A       max7219_SpiBuff[i] = Word;
	RCALL SUBOPT_0x8
	RCALL SUBOPT_0xD
	RCALL SUBOPT_0xC
	ST   Z,R16
	STD  Z+1,R17
; 0003 002B     else                  // Нужно записать только в один max7219
	RJMP _0x60007
_0x60006:
; 0003 002C     {
; 0003 002D       if (i == MAX_Idx)         // Та микросхема max7219, в которую нужно записать команду/данные
	LDD  R30,Y+14
	CP   R30,R19
	BRNE _0x60008
; 0003 002E         max7219_SpiBuff[i] = Word;
	RCALL SUBOPT_0x8
	RCALL SUBOPT_0xD
	RCALL SUBOPT_0xC
	ST   Z,R16
	STD  Z+1,R17
; 0003 002F       else                      // max7219, которому нет данных на запись
	RJMP _0x60009
_0x60008:
; 0003 0030         max7219_SpiBuff[i] = 0x00 | ((uint16_t) MAX7219_CMD_NO_OP << 8);
	RCALL SUBOPT_0x8
	RCALL SUBOPT_0xD
	LSL  R30
	ROL  R31
...
_0x60005:
; 0003 0033 
; 0003 0034   // Пишем пачку слов во все подключенные max7219
; 0003 0035   max7219_sendarray(max7219_SpiBuff);
	RCALL SUBOPT_0xD
	RCALL _max7219_sendarray
; 0003 0036 }
	RCALL __LOADLOCR4
...
;	i -> R17
	CBI  0x18,0
; 0003 0041 
; 0003 0042   for (i = 0; i < MAX7219_NUM; i++)
	LDI  R17,LOW(0)
_0x6000B:
	CPI  R17,4
	BRSH _0x6000C
; 0003 0043     max7219_send16bit(*(pArray++));
	LDD  R26,Y+1
	LDD  R27,Y+1+1
	LD   R30,X+
	LD   R31,X+
...
; 0003 004F   // Выдаём старший байт
; 0003 0050   spi(hibyte(Word));
	RCALL SUBOPT_0xE
;	Word -> Y+0
	LDD  R30,Y+1
	ANDI R31,HIGH(0x0)
	MOV  R26,R30
	RCALL _spi
; 0003 0051   // Выдаём младший байт
; 0003 0052   spi(lobyte(Word));
	LD   R30,Y
	MOV  R26,R30
	RCALL _spi
; 0003 0053 }
...
; 0003 005C   max7219_send(MAX_Idx, MAX7219_CMD_DECODE_MODE, DecodeMode);
	ST   -Y,R26
;	MAX_Idx -> Y+1
;	DecodeMode -> Y+0
	RCALL SUBOPT_0xF
...
;	Intensity -> Y+0
	...
	CPI  R26,LOW(0x10)
	BRLO _0x6000D
; 0003 0067     Intensity = 15;
	LDI  R30,LOW(15)
	ST   Y,R30
; 0003 0068 
; 0003 0069   max7219_send(MAX_Idx, MAX7219_CMD_INTENSITY, Intensity);
_0x6000D:
	RCALL SUBOPT_0xF
	LDI  R30,LOW(10)
	RCALL SUBOPT_0x10
...
; 0003 0074     Limit = 7;
	...
	ST   Y,R30
; 0003 0075 
; 0003 0076   max7219_send(MAX_Idx, MAX7219_CMD_SCAN_LIMIT, Limit);
_0x6000E:
	RCALL SUBOPT_0xF
	LDI  R30,LOW(11)
	RCALL SUBOPT_0x10
...
; 0003 0081     On = 1;
	...
	ST   Y,R30
; 0003 0082 
; 0003 0083   max7219_send(MAX_Idx, MAX7219_CMD_SHUTDOWN, On);
_0x6000F:
	RCALL SUBOPT_0xF
	LDI  R30,LOW(12)
	RCALL SUBOPT_0x10
...
; 0003 008E     On = 1;
	...
	ST   Y,R30
; 0003 008F 
; 0003 0090   max7219_send(MAX_Idx, MAX7219_CMD_DISP_TEST, On);
_0x60010:
	RCALL SUBOPT_0xF
	LDI  R30,LOW(15)
	RCALL SUBOPT_0x10
